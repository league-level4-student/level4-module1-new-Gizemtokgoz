...
public class PolymorphWindow extends JPanel implements ActionListener {
    ...
    static Polymorph moveMorpth;
    static Polymorph clickMorph;
    static ArrayList<Polymorph> polmop = new ArrayList<Polymorph>();

    public static void main(String[] args) {
        new PolymorphWindow().buildWindow();
        polmop.add(bluePoly);
        polmop.add(redPoly);
        polmop.add(moveMorpth);
        polmop.add(clickMorph);
    }

    public void buildWindow() {
    	for (int i = 0; i < polmop.size(); i++) {
			polmop.get(i).draw(getGraphics());
			polmop.get(i).update();
		}
